...
class _EditFriendListScreenState extends ConsumerState<EditFriendListScreen> {
  ...
  void removeFriend(String friendUID) {
    ...
    final friendProfile = friendProfiles[friendUID];
    final myUID = FirebaseAuth.instance.currentUser!.uid;
    final myUsername = ref.read(profileProvider).name;
    final myIconLink = ref.read(profileProvider).iconLink;
    FirestoreHelper().addNotification(
      'Friend Removed',
      '${friendProfile!.name} is removed from your friend list.',
      friendProfile.iconLink!,
      'Friend Removed',
      [myUID],
      myUID,
    );
    FirestoreHelper().addNotification(
      'Friend Removed',
      '$myUsername is removed from your friend list.',
      myIconLink ?? Statics.defaultIconLink,
      'Friend Removed',
      [friendUID],
      myUID,
    );
                ...
                setState(() {
                  if (value == true) {
                    if (!muteList.contains(friendUID)) {
                      ...
                      FirestoreHelper().addMutedList(friendUID);
                    }
                  } else {
                    if (muteList.contains(friendUID)) {
                      muteList.remove(friendUID);
                      PrefsHelper().removeMutePrefs(friendUID);
                      FirestoreHelper().removeMutedList(friendUID);
                    }
                  }
                });
              },
